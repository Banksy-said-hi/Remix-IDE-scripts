...
contract Dong {
    ...
    constructor(
        ...
        // RemiaingDongInMatic has two decimals for precision
        remainingDongInMatic = totalDollarAmount*maticPriceDecimal*dongDecimal/maticPrice;
    ...
    function payDong(string calldata _name) public payable {
        require(int(msg.value) == dongInMatic, "msg.value must be at least equal to dong");
        require(finished == false, "The process has already been finished");

        remainingDongInMatic -= int(msg.value);
        payment[msg.sender] += int(msg.value);

        names[counter] = _name;
        counter += 1;

        if (counter == uint(contributors)) {
